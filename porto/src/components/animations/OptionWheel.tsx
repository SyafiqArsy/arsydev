...
interface OptionWheelProps {
  ...
  /** Called whenever the wheel settles on a new option (scroll/drag). */
  onChange?: (index: number, item: string) => void;
  /** Called when an option is explicitly chosen by clicking it. */
  ...
  curve?: number;
  /** Angle in degrees between neighboring options. */
  tilt?: number;
  ...
  smoothing?: number;
  /** Padding between the anchored edge and the centered option. */
  inset?: number;
  /** Wrap around infinitely instead of stopping at the ends. */
  loop?: boolean;
  /** Allow dragging with a pointer, in addition to scroll and arrow keys. */
  draggable?: boolean;
  /** URL of a short tick sound; empty disables it. */
  soundUrl?: string;
...
export default function OptionWheel({
  ...
  // Keep latest values readable from the rAF loop and handlers so listeners
  // never need to be torn down and re-registered.
  useEffect(() => {
  ...
  // rAF loop disimpan di ref agar bisa merujuk dirinya sendiri tanpa
  // melanggar aturan react-hooks/immutability (const yang disebut lebih dulu).
  ...
  useEffect(() => {
    runFrameRef.current = (now: number) => {
      ...
      lastRef.current = now;
      const cfg = cfgRef.current as Config;
      ...
      // neighbors equal to one row height, so the reference controls how tightly it curls.
  ...
  const playTick = useCallback(() => {
    const { soundUrl, soundVolume } = cfgRef.current as Config;
    if (!soundUrl) return;
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    if (!(cfgRef.current as Config).draggable) return;
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (dragMovedRef.current) applyTarget(drag.start - dy / (cfgRef.current as Config).rowH, false);
    },
    [applyTarget],
  );
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      // Klik pada seseor pada roda → pilihan dikonfirmasi (navigasi).
