...
const ScrollStack = ({
  ...
  const lastTransformsRef = useRef<Map<number, StackTransform>>(new Map());
  ...
  const setupLenis = useCallback(() => {
    if (!enableSmooth) {
      // Tanpa smooth-scroll internal: pakai window scroll langsung.
      // Update transforms saat window scroll (dipanggil setupLenis via useLayoutEffect).
    ...
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        wrapper: scroller,
        content:
          scroller.querySelector<HTMLElement>(".scroll-stack-inner") ?? undefined,
        duration: 1.2,
        easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 2,
